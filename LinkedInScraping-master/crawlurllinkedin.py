...
search_query = input('What Keyword do you want to scrape? ')
search_field.send_keys(search_query)

# Task 2.3: Search
search_field.send_keys(Keys.RETURN)
search_query1 = search_query.replace(" ",'%20')
driver.get('https://www.linkedin.com/search/results/people/?keywords='+ search_query1 + '&origin=SWITCH_SEARCH_VERTICAL&sid=1*Y')
...
# search_keyLst = open(r'F:\PycharmProjects\Source\LinkedInScraping-master\credential\credentials5.txt')
# line = search_keyLst.readlines()
# for i in tqdm(range(len(line))):
#     check = True
#     sleep(7)
#     #driver.get('https://www.linkedin.com/feed/')
#     #sleep(2)
#     text_search = line[i].replace('\n','')
#     text_search = text_search.replace(' ','%20')
#     text = 'https://www.linkedin.com/search/results/people/?geoUrn=%5B%2290010187%22%2C%22103697962%22%5D&keywords=' + text_search + '&network=%5B"F"%2C"S"%2C"O"%5D&origin=GLOBAL_SEARCH_HEADER'
#     print(text)
#     #search_field = driver.find_element_by_xpath('//*[@class="search-global-typeahead__input always-show-placeholder"]')
#     #search_field.send_keys(Keys.RETURN)
#     driver.get(text)
...
for page in range(input_page):
    ...
    try:
        ...
        if len(URLs_all_page) == checklengt:
            link = driver.current_url
            driver.get(link)
            sleep(1)
            checklengt = len(URLs_all_page)
            pass
        else:
            checklengt = len(URLs_all_page)
            pass
