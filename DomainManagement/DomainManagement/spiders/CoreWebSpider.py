...
def outer_func():
    ...
    async def initspider1(url, selector, keyword, deepscan, linkadd):
        ...
        list_child_url.append(url_obj(url, 0, 1, selector, keyword, deepscan, linkadd, start_time))
        ...
        # finish, unfinish = await asyncio.wait(task)
        # print(finish)
        # if task in finish:
    ...
    async def takeparam(param):
        ...
        for data in tqdm(dataparam):
            ...
            addurl = data['Url']
            task  = asyncio.create_task(initspider1(param1, selector, checkwordlist, deepscan, addurl))
            await asyncio.gather(task)
