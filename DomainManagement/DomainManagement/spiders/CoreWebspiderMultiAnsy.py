...
class url_obj:
    ...
    #              Textcontent):
    def __init__(self, url, iscan, deep, selector, keyword, deepscan, linkadd, StartTime, SessionCode,RobotCode,SetData):
        self.url = url
        self.iscan = iscan
        self.deep = deep
                            ...
                            if self.chk_link_exist(link) == 0:
                                ...
                                self.list_child_url.append(
                                    url_obj(url, iscan, deep, self.selector, self.keyword, self.deepscan,
                                            self.linkadd, self.StartTime, self.SessionCode,self.RobotCode,self.SetData))
    ...
    def upload_data(self, Document):
        ...
        filename = self.sessioncode[0] + '_' + mystr[0:5] + '.docx'
        ...
        data = {
            'SessionCode': self.sessioncode[0],
    ...
    async def main(self):
        ...
        #object = url_obj(url, 0, 1, selector, keyword, deepscan, linkadd, start_time,SessionCode,botCode,SetData)
        self.list_child_url.append(url_obj(SetData[0], SetData[1], SetData[2], SetData[3], SetData[4], SetData[5],
                                      SetData[5], SetData[6], SetData[7], SetData[8], SetData[9]))
        Idx = 0
        ...
        for x in self.keyword:
            self.Lstkeyword.append(x)
        self.sessioncode.clear()
        self.sessioncode.append(self.SessionCode)
...
async def initspider1(url, selector, keyword, deepscan, linkadd ,SessionCode,botCode):
    ...
    SetData.append(0)
    SetData.append(1)
    SetData.append(selector)
    SetData.append(keyword)
    SetData.append(deepscan)
    SetData.append(linkadd)
    SetData.append(start_time)
    SetData.append(SessionCode)
    SetData.append(botCode)
    SetData.append(SetData)
    print(SetData)
    object = url_obj(url, 0, 1, selector, keyword, deepscan, linkadd, start_time,SessionCode,botCode,SetData)
...
async def takeparam(param):
    ...
    for data in tqdm(dataparam):
        ...
        print(checkwordlist)
        botCode = data['RobotCode']
        sessionCode = data['BotSessionCode']
        #sesioncode.append(sessionCode)
        myUUID = uuid.uuid4()
        mystr = str(myUUID)
        SessionCode = sessionCode
        addurl = data['Url']
        task  = asyncio.create_task(initspider1(param1, selector, checkwordlist, deepscan, addurl,SessionCode,botCode))
        await asyncio.gather(task)
...
async def listen():
    ...
    print("oke1")
    ws_connect = websockets.connect('ws://117.6.131.222:9095', ping_interval=None)
    print("oke2")
    my_task = None
    task_param = None
